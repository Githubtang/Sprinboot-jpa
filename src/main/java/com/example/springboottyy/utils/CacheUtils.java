...
public class CacheUtils {
    ...
    public static Set<String> getKeys(String cacheName) {
        ...
        CacheKeys cacheGetKeys = SpringUtils.getBean(CacheKeys.class);
        return cacheGetKeys.getCacheKeys(cache);
    }
     ...
     * @param <T>
     */
    public static <T> void put(String cacheName, String key, T value, long timeout, TimeUnit timeUnit) {
        Cache cache = getCacheManager().getCache(cacheName);
        if (cache instanceof JCacheCache){
            ...
            if (timeout != 0 && timeUnit != null){
                cacheTimeOut.setCacheObject(cacheName,key,value,timeout,timeUnit);
            }else {
